...
# Function, .................................................
def prepare_list_with_subset_collection_composition_list(*, 
            subset_collection_names,       # names of subsets collection varinats, that have the same module, and dataset name and variant but different composition of batches,
            subset_collection_composition, # list with dict, where one of values is df wiht subset composition 
    ...
    '''
        this is a function that prepares dataframes with name of files 
        to load with differen batches of data and their labels, that were grouped in so called subset collections
        eg collections with train valid and test data, each with one batch of data may be use to select best parameters,
        whereas large collections wiht many batches in each of these categories may be used for final model training    
    '''
    
    dict_to_return = dict()
                                              
    for subset_collection_name in subset_collection_names:                                       
        input_data_summary_df = pd.DataFrame(subset_collection_composition)
        df_bool_idx   = ((  input_data_summary_df.dataset_name     == dataset_name)
              & (input_data_summary_df.dataset_variant             == dataset_variant)
              & (input_data_summary_df.module_name                 == module_name)
             & (input_data_summary_df.subset_collection_name       == subset_collection_name))
        subset_collections_to_load_df = input_data_summary_df.dataset_compositions_df.loc[np.where(df_bool_idx.values==True)[0][0]]
        dict_to_return[subset_collection_name] = subset_collections_to_load_df # extra step for clarity, 
...
# Function, .................................................
def train_and_test_sklearn_models(*,
    ...
    train_proportion=0.8,  # used only if validation datasets is not specified, 
    ...
    for cycle_nr, subset_collection_name in enumerate(subset_collection_names):
        ...
        selected_data_subsets_role = data_subsets_role[subset_collection_name]
        ...
        if isinstance(selected_data_subsets_role["valid"], float):
            ...
            selected_data_subsets_role["valid"] = None # to avoid problems    
        else:
        ...
        if cycle_nr==0:
            ...
            for gidx in best_methods_IDs:
                cycle_grid.append(model_parameter_list[gidx]['para  ms']) # yes its 'para  ms'! its a mistake, that I have no time to correct 

        
        # ...............................................
        # run grid search, 
        results_list, model_predictions_dict, model_parameter_list = sklearn_grid_search(
            ...
            unit_test             = unit_test,
            train_proportion      = train_proportion_applied,
    ...
    return results_list, model_parameter_list

            
            

# Function, ........................................................
def train_skilearn_models_iteratively(*, 
    # names 
    run_ID="run01",
    dataset_name,
    dataset_variant,
    ...
    results_list, model_parameter_list = train_and_test_sklearn_models(  
